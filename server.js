...
wss.on("connection",ws=>{let id=null;ws.isAlive=true;ws.on("pong",()=>{ws.isAlive=true});ws.on("message",raw=>{let m;try{m=JSON.parse(raw)}catch{return}
 ...
 const me=db.devices[id];if(m.type==="ping")return send(ws,{type:"pong"});if(!me)return;
 ...
 if(m.type==="chat"){if(!me.pair)return send(ws,{type:"sendError",message:"Not paired on server. Please pair this device again."});const mid=String(m.messageId||uid());let msg=db.messages.find(x=>x.id===mid);if(msg){send(ws,{type:"accepted",messageId:mid});const p=db.devices[msg.to];if(online(p)&&msg.status==="sent"){send(p.ws,{type:"message",...msg,fromName:db.devices[msg.from]?.name||"Device"});msg.status="delivered";save();send(ws,{type:"delivered",messageId:mid})}else if(msg.status==="delivered")send(ws,{type:"delivered",messageId:mid});else if(msg.status==="read")send(ws,{type:"read",messageId:mid});return}msg={id:mid,text:String(m.text||"").trim().slice(0,4000),from:id,to:me.pair,time:Date.now(),status:"sent"};if(!msg.text)return;db.messages.push(msg);save();send(ws,{type:"accepted",messageId:mid});const p=db.devices[me.pair];if(online(p)){send(p.ws,{type:"message",...msg,fromName:me.name});msg.status="delivered";save();send(ws,{type:"delivered",messageId:mid})}return}
...
const heartbeat=setInterval(()=>{wss.clients.forEach(ws=>{if(ws.isAlive===false)return ws.terminate();ws.isAlive=false;ws.ping()})},25000);
server.listen(PORT,'0.0.0.0',()=>console.log(`CADVORA Private Chat V12 running on port ${PORT}`));
